...
def make_batch(train_path, word2number_dict, batch_size, n_step):
    ...
    for sen in text:
        ...
        if len(word) <= n_step:  # pad the sentence
            word = word + ["<pad>"] * (n_step + 1 - len(word))
...
class TextLSTM(nn.Module):
    def __init__(self):
        ...
        self.embedding = nn.Embedding(n_class, emb_size, padding_idx=0)  # 词嵌入矩阵
        # i_t
        self.W_ii = nn.Linear(emb_size, n_hidden)  # [256,128]
        ...
        self.W_ii1 = nn.Linear(emb_size, n_hidden)  # [256,128]
        self.b_ii1 = nn.Parameter(torch.ones([n_hidden]))
        self.W_hi1 = nn.Linear(n_hidden, n_hidden)
        self.b_hi1 = nn.Parameter(torch.ones([n_hidden]))
        # f_t
        self.W_if = nn.Linear(emb_size, n_hidden)
        ...
        self.W_if1 = nn.Linear(emb_size, n_hidden)
        self.b_if1 = nn.Parameter(torch.ones([n_hidden]))
        self.W_hf1 = nn.Linear(n_hidden, n_hidden)
        self.b_hf1 = nn.Parameter(torch.ones([n_hidden]))
        # g_t
        self.W_ig = nn.Linear(emb_size, n_hidden)
        self.b_ig = nn.Parameter(torch.ones([n_hidden]))
        self.W_hg = nn.Linear(n_hidden, n_hidden)
        self.b_hg = nn.Parameter(torch.ones([n_hidden]))
        self.W_ig1 = nn.Linear(emb_size, n_hidden)
        self.b_ig1 = nn.Parameter(torch.ones([n_hidden]))
        self.W_hg1 = nn.Linear(n_hidden, n_hidden)
        self.b_hg1 = nn.Parameter(torch.ones([n_hidden]))
        # o_t
        self.W_io = nn.Linear(emb_size, n_hidden)
        ...
        self.W_io1 = nn.Linear(emb_size, n_hidden)
        self.b_io1 = nn.Parameter(torch.ones([n_hidden]))
        self.W_ho1 = nn.Linear(n_hidden, n_hidden)
        self.b_ho1 = nn.Parameter(torch.ones([n_hidden]))

        # self.W_xh = nn.Parameter(torch.Tensor(n_hidden,n_hidden))
        # self.W_hh = nn.Parameter(torch.Tensor(n_hidden,n_hidden))
    ...
    def forward(self, X, hidden, cell):
        h_t_1 = hidden[0].unsqueeze(0)
        c_t_1 = cell[0].unsqueeze(0)
        h_t_11 = hidden[1].unsqueeze(0)
        c_t_11 = cell[1].unsqueeze(0)
        X = self.embedding(X)
        # print(X.shape)
        ...
        for x_t in X:  # x_t[128,256]
            ...
            i_t1 = self.sigmoid(self.W_ii1(h_t[-1]) + self.b_ii1 + self.W_hi1(h_t_11) + self.b_hi1)
            f_t1 = self.sigmoid(self.W_if1(h_t[-1]) + self.b_if1 + self.W_hf1(h_t_11) + self.b_hf1)
            g_t1 = self.tanh(self.W_ig1(h_t[-1]) + self.b_ig1 + self.W_hg1(h_t_11) + self.b_hg1)
            o_t1 = self.sigmoid(self.W_io1(h_t[-1]) + self.b_io1 + self.W_ho1(h_t_11) + self.b_ho1)
            c_t1 = f_t1 * c_t_11 + i_t1 * g_t1
            h_t1 = o_t1 * self.tanh(c_t1)

            h_t_11 = h_t1
            c_t_11 = c_t1
            output = torch.cat((h_t, h_t1), 2)
            outputs.append(output[-1])

            outputs = torch.tensor([item.detach().numpy() for item in outputs])
            outputs = outputs[-1]
...
def train_LSTMlm():
    ...
    for epoch in range(all_epoch):
        ...
        for input_batch, target_batch in zip(all_input_batch, all_target_batch):
            optimizer.zero_grad()
            hidden_state = torch.zeros(2, batch_size,
                                       n_hidden)  # [num_layers(=1) * num_directions(=1), batch_size, n_hidden]
            cell_state = torch.zeros(2, batch_size, n_hidden)  # [num_layers(=1) * num_directions(=1), b
        ...
        with torch.no_grad():
            ...
            for valid_batch, valid_target in zip(all_valid_batch, all_valid_target):
                hidden_state = torch.zeros(2, batch_size, n_hidden)
                cell_state = torch.zeros(2, batch_size, n_hidden)
        ...
        if (epoch + 1) % save_checkpoint_epoch == 0:
            torch.save(model, f'models/fan-LSTMlm_model_epoch{epoch + 1}.ckpt')
...
if __name__ == '__main__':
    ...
    # data_root = '/Users/huanglei15/vscodeProjects/nlp_homework/NLP2021_homework/penn_small'
    ...
    print("\nTest the LSTMLM……………………")
    select_model_path = "models/fan-LSTMlm_model_epoch5.ckpt"
